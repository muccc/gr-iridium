...
fft_channelizer_impl::fft_channelizer_impl(int fft_size, int decimation, bool activate_streams, int pdu_ports)
    : gr::sync_decimator("fft_channelizer",
                         ...
                         gr::io_signature::make(
                             activate_streams ? decimation + 1 : 0 /* min outputs */,
                             activate_streams ? decimation + 1 : 0 /* max outputs */, sizeof(output_type)),
                         decimation),
...
void fft_channelizer_impl::burst_handled(pmt::pmt_t msg)
{
    d_outstanding--;
}
...
{
    uint64_t ret = 0;
    float relative_frequency = pmt::to_float(pmt::dict_ref(new_burst.value, pmt::mp("relative_frequency"), pmt::PMT_NIL));

    for(int i = 0; i < d_channels; i++) {
        if(channel_lower_border(i) < relative_frequency && relative_frequency < channel_upper_border(i)) {
            ret |= (1<<i);
        }
    }
    return ret;
...
{
    uint64_t id = pmt::to_uint64(pmt::dict_ref(new_burst.value, pmt::mp("id"), pmt::PMT_NIL));
    float magnitude = pmt::to_float(pmt::dict_ref(new_burst.value, pmt::mp("magnitude"), pmt::PMT_NIL));
    double center_frequency = pmt::to_double(pmt::dict_ref(new_burst.value, pmt::mp("center_frequency"), pmt::PMT_NIL));
    float sample_rate = pmt::to_float(pmt::dict_ref(new_burst.value, pmt::mp("sample_rate"), pmt::PMT_NIL));
    float relative_frequency = pmt::to_float(pmt::dict_ref(new_burst.value, pmt::mp("relative_frequency"), pmt::PMT_NIL));
    uint64_t timestamp = pmt::to_uint64(pmt::dict_ref(new_burst.value, pmt::mp("timestamp"), pmt::PMT_NIL));
    float noise = pmt::to_float(pmt::dict_ref(new_burst.value, pmt::mp("noise"), pmt::PMT_NIL));
    ...
    burst_data burst = {id, (double)new_burst.offset / d_decimation, magnitude, relative_frequency,
    center_frequency, sample_rate, timestamp, noise, 0};
    burst.data = (gr_complex *) malloc(sizeof(gr_complex) * d_max_burst_size);
    return burst;
}

void fft_channelizer_impl::append_to_burst(burst_data &burst, const gr_complex * data, size_t n)
{
                               ...
                               gr_vector_void_star& output_items)
{
    ...
    for(int i = 0; i < ninput_items; i += d_fft_size - d_fft_size / d_inverse_overlap) {
        ...
        // No stream outputs, no channel active, no new channel becoming active => No need to generate any output
        ...
        get_tags_in_window(gone_bursts, 0, i, i + d_fft_size - d_fft_size / d_inverse_overlap, pmt::mp("gone_burst"));
        ...
        for(int j = 0; j < d_channels; j++) {
            ...
            if(output_items.size() > 0 || d_channel_active & (1 << j)) {
                ...
                volk_32fc_s32fc_multiply_32fc(&d_ifft.get_inbuf()[0], &d_fft.get_outbuf()[positive_offset(d_fft_size, d_output_step, j, d_ifft_size)], 1./d_fft_size, d_ifft_size / 2);
                volk_32fc_s32fc_multiply_32fc(&d_ifft.get_inbuf()[d_ifft_size / 2], &d_fft.get_outbuf()[negative_offset(d_fft_size, d_output_step, j, d_ifft_size)], 1./d_fft_size, d_ifft_size / 2);
                d_ifft.execute();

                if(output_items.size() > 0) {
                    output_type* out = reinterpret_cast<output_type*>(output_items[j]);
                    memcpy(&out[i / d_decimation], &d_ifft.get_outbuf()[d_ifft_size / d_inverse_overlap], (d_ifft_size - d_ifft_size / d_inverse_overlap) * sizeof(gr_complex));
                }

                if(d_pdu_ports > 0) {
                    // Fill already active bursts with samples
                    for(auto& kv : d_bursts[j]) {
                        append_to_burst(kv.second, &d_ifft.get_outbuf()[d_ifft_size / d_inverse_overlap], (d_ifft_size - d_ifft_size / d_inverse_overlap));
                    }

                    // Create new bursts. As new new bursts might start in the middle of this block,
                    // the already active bursts get their data before creating the new ones.
                    for(tag_t new_burst : new_bursts) {
                        if(activated_channels(new_burst) & (1 << j)) {
                            ...
                            int relative_offset = (new_burst.offset - nitems_read(0) - i) / d_decimation;
                            int to_copy = max_items - relative_offset;
                            append_to_burst(d_bursts[j][burst.id], &d_ifft.get_outbuf()[d_ifft_size / d_inverse_overlap + relative_offset], to_copy);
                        }
                    }
